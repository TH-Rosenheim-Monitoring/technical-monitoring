...
def plot_conditional_marking(cond,color="yellow",alpha=.5,ax=None,limits=None,label="_hidden",b_use_fill_between=True):
...
def df_clusterTime_function(d,function=None):
    """Determination of the time intervals at which a specific condition (e.g. threshold exceeded) occurs and calculation of the desired quantity (e.g. average value, length, etc.) for each time interval.
        Arguments: 
            ...
            function: function to apply on identified cluster values to calculate the clusterRepresentation value. Can be one of the following:
                - custom function
                -a string with value "function_cusmum_not_nan" -> the length of the cluster in time steps will be calculated
                if no value is given, the mean value in the cluster is calculated
        Returns: pandas time series with the actual values in the DataFrame replaced by the calculated cluster representation value
    """
    d=d.copy(deep=True)
    ...
    dd=dd[~pd.isna(d)]
    dd.columns+="_clusterRepresentationValue"
    return(dd)
